...
def GenerateRTPpacket(rtp_params: dict[str, Any]):
    ##Example Usage:
    ...
    byte1 = format(
        int((version + padding + extension + csi_count), 2), "x"
    ).zfill(
        2
...
def DecodeRTPpacket(packet_bytes: bytes):
    ##Example Usage:
